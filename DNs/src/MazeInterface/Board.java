...
public class Board extends JPanel implements Commons {
	...
	private boolean test_where_what_flag  = where_what_flag;
    ...
    private int where_what_total = 1000;
    private int train_where_what_count = 0;
    private int test_where_what_count = 0;
	...
	//add iteration for skill training
	int iter = 0;
	int[] initial_random = {-3, 0, 3};
	...
	private void gameInit() {
		if (learn_count == lesson_iteration && lesson_iteration != 0) {
			...
			//add iteration for skill training   && (current_lesson == schedule.getNumSkills())
			if(iter<1 && (current_lesson == 2)) {
				current_lesson = 0;
				iter += 1;
			}
		...
		if(current_lesson - schedule.getNumSkills()<0) {
			agent.setInitAgentLoc(map.getStartingLocS(initial_random[iter],0), map.getStartingGpsAngle());
		}
		else {
			...
			agent.setInitAgentLoc(map.getStartingLocS(rw,rh), map.getStartingGpsAngle());
		}
	...
	private void drawObjects(Graphics2D g2d) {
		...
		if (train_where_what_flag || test_where_what_flag){
			String string_where_what = "Current Loc: " + location[current_loc_id];
			currentY += VISIONLINESPACE;
			...
			string_where_what = "Current Scale: " + scale[current_scale_id];
			currentY += VISIONLINESPACE;
			g2d.drawString(string_where_what, VISIONTITLEX, currentY);
	...
	private class ScheduleTask extends TimerTask {
		...
		public void run() {
			if (train_where_what_flag && !paused){
				updateWhereWhatVision();
				env current_type = env.values()[foreground_type[current_type_id]];
				int current_loc  = location[current_loc_id];
				int current_scale = scale[current_scale_id];
	...
	private void updateWhereWhatVision() {
		...
		for (int i = 0; i < Agent.vision_num; i++){
			...
			float current_vision_length = randFloat(agent.safe_distance, Agent.vision_range);
		...
		int starting_loc = location[current_loc_id];
		int ending_loc = starting_loc + scale[current_scale_id];
		env current_vision_type = env.values()[foreground_type[current_type_id]];
		float current_vision_length = randFloat(agent.safe_distance, Agent.vision_range);
		System.out.println("starting loc: " + starting_loc + ", ending loc: " + ending_loc);
		for (int i = starting_loc; i < ending_loc; i++){
			float angle = agent.angle + (float)Agent.visino_angle[i];
			visions[i].setLength(current_vision_length, angle);
			visions[i].setType(current_vision_type);
		}
				...
				if (get_back_flag == false){
				    ...
				    if (current_lesson < schedule.getNumLessons() - 1){
				        agent.learnPlanning(map.getPlanSequence(), current_lesson - schedule.getNumSkills(), map.getValue(), 
				         		            current_distance);
				        if (means_learned_flag[0] == true && means_learned_flag[1] == true){
